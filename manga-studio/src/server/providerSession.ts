...
const agentTypes = ["openai-compatible", "anthropic-compatible", "gemini"] as const;
// "generic-rest" is a legacy alias for openai-compatible image endpoints.
const imageTypes = ["gemini", "openai-compatible", "generic-rest"] as const;
...
export function readSessionConfig(request: NextRequest, kind: ProviderKind): ProviderConfig | null {
  ...
  try {
    ...
    return parsed.kind === kind && parsed.apiKey && parsed.baseUrl && parsed.model ? parsed : null;
  } catch {
    return null;
  }
}

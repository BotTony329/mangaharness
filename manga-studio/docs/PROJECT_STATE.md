...
- Exact existing-asset route acceptance: passed. Request `03868b20-20f6-462d-83ab-187a38250dff` loaded the original 710,104-byte 848×1264 Cute Girl jumping JPEG, completed `built-in-connectivity:checkerboard-matte`, validated real alpha, stored a separate Blob PNG, and returned HTTP 200 in 2.74 s. The derivative measured 79.12% transparent and 20.88% opaque pixels.
- Production UI reprocess acceptance: passed. The exact opaque source was uploaded as a Character reference, automatically processed, and then manually reprocessed without regeneration. Request `5f151390-c9bf-4d0b-bf5a-231cf4396792` returned ready in 1.57 s; the browser logged `asset_updated` for the same asset ID and displayed the transparent derivative over the CSS preview checkerboard. No browser/page errors were reported.
- Production canvas/export acceptance: passed. The repaired Cute Girl was dragged into Panel 1 above a coloured scene; the scene remained visible around hair, raised arms, body, skirt, legs, and shoes with no opaque rectangle. The downloaded `my-manga-page-1@1x.png` preserved that composition and contained no checkerboard preview pixels.

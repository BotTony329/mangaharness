...
| Transparent character assets | WORKING LOCALLY; DEPLOYMENT PENDING | Generated/uploaded character and prop sources use an ordered cascade: native alpha → image-provider edit → dedicated removal provider → conservative local fallback. Every candidate is decoded and checked for meaningful alpha, visible foreground, background removal, and usable bounds before a derivative is promoted. |
| Background removal | WORKING LOCALLY; DEPLOYMENT PENDING | Background Removal is an independent BYOK capability with its own encrypted cookie, remove.bg Quick Preset, Custom API mapping, safe logs/errors, and optional deployment fallback. Manual recovery offers Retry, Image AI, provider selection, Keep Raw, and safe Details. |
